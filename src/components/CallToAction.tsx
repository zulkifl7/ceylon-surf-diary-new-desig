  ...
  return (
    <section ref={elementRef} className="py-20 px-6" style={{ backgroundColor: '#FCFCFC' }}>
      <div className="max-w-4xl mx-auto">
        <div className={`flex flex-col md:flex-row md:items-center md:justify-between gap-8 transition-all duration-1000 ease-out ${
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
        }`}>
          <div className="flex-1">
            <h2 className={`text-5xl mb-6 font-jedira tracking-wide transition-all duration-1000 ease-out ${
              isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'
            }`} style={{ transitionDelay: isVisible ? '200ms' : '0ms' }}>
              Make it your way!
            </h2>
            <p className={`text-gray-600 font-poppins text-lg transition-all duration-1000 ease-out ${
              isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'
            }`} style={{ lineHeight: '1.8', transitionDelay: isVisible ? '400ms' : '0ms' }}>
